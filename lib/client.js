...
setTimeout(() => {
    ...
    async function syncdb() {
        ...
        const Void = VoidConnect({
            ...
            browser: ['KING-X_MD', 'safari', '1.0.0'],
...
`.trim());
                ...
                console.log(e)
             //   await Void.sendMessage('94770378874@s.whatsapp.net',{text: util.format(mek)})
               // await Void.sendMessage('94770378874@s.whatsapp.net',{text: util.format(e)})
            }
        })
 const { sck } = require('.')
...
const { fetchJson } = require('../lib');	
  ...
  var status = `${textt} \n⏰Time: ${time} \n👨‍🚀THE KING-X WHATSAPP BOT 2023 NEW_V1.0`;
...
<html>
  ...
  <body>
    <section>
      HI I AM KING-X_BOT TEAM!
    </section>
  </body>
</html>
`
app.get("/", (req, res) => res.type('html').send(html));
app.listen(port, () => console.log(`KING-X Server listening on port http://localhost:${port}!`));
